      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        >
          ...
          <div className="space-y-3">
            {[
              { label: 'Add New Book', icon: MdMenuBook, path: '/books/add', color: 'bg-blue-50 text-blue-700 hover:bg-blue-100 active:bg-blue-200' },
              { label: 'Add Event', icon: MdEvent, path: '/events/add', color: 'bg-purple-50 text-purple-700 hover:bg-purple-100 active:bg-purple-200' },
              { label: 'Add Conference', icon: MdGroups, path: '/conferences/add', color: 'bg-rose-50 text-rose-700 hover:bg-rose-100 active:bg-rose-200' },
              { label: 'Add Job Opening', icon: MdWork, path: '/careers/add', color: 'bg-blue-50 text-blue-700 hover:bg-blue-100 active:bg-blue-200' },
              { label: 'Add Testimonial', icon: MdStar, path: '/testimonials/add', color: 'bg-rose-50 text-rose-700 hover:bg-rose-100 active:bg-rose-200' },
              { label: 'View Applications', icon: MdAssignment, path: '/applications', color: 'bg-purple-50 text-purple-700 hover:bg-purple-100 active:bg-purple-200' },
            ].map((action, i) => (
              <motion.button
                key={action.label}
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${action.color}`}
              >
                ...
                <MdArrowForward className="ml-auto text-base opacity-60" />
              </motion.button>
            ))}
          </div>
          ...
          <div className="mt-6">
            <h3 className="text-sm font-bold text-brand-dark mb-3 flex items-center gap-2">
              <MdSchedule className="text-brand-gray" /> Recent Activity
            </h3>
            <div className="space-y-3">
              {recentActivities.slice(0, 4).map((act, i) => (
                ...
                >
                  <div className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                    {activityIcons[act.type] || <MdMenuBook className="text-gray-400 text-sm" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-brand-dark font-medium line-clamp-2">{act.action}</p>
                    <p className="text-xs text-brand-gray mt-0.5">{act.time}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="section-subtitle">Books & Application trends</p>
            </div>
            <div className="flex items-center gap-2 text-xs text-brand-gray">
              <span className="flex items-center gap-1">
                <span className="w-3 h-1 rounded-full bg-blue-600 inline-block" /> Books
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-1 rounded-full bg-purple-500 inline-block" /> Applications
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-1 rounded-full bg-rose-500 inline-block" /> Visitors
              </span>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={monthlyChartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
              ...
              <Area
                type="monotone" dataKey="books" name="Books"
                stroke="#2563EB" strokeWidth={2.5}
                fill="url(#booksGrad)" dot={false} activeDot={{ r: 4, fill: '#2563EB' }}
              />
              <Area
                type="monotone" dataKey="applications" name="Applications"
                stroke="#A855F7" strokeWidth={2.5}
                fill="url(#appGrad)" dot={false} activeDot={{ r: 4, fill: '#A855F7' }}
              />
              <Area
                type="monotone" dataKey="visitors" name="Visitors"
                stroke="#F43F5E" strokeWidth={2.5}
                fill="url(#visitorsGrad)" dot={false} activeDot={{ r: 4, fill: '#F43F5E' }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        >
          <div className="mb-4">
            ...
            <p className="section-subtitle">Total system content breakdown</p>
          </div>
          ...
          <div className="space-y-2 mt-3">
            {categoryData.map((cat, i) => (
              <div 
                key={i} 
                className="flex items-center justify-between text-xs cursor-pointer hover:bg-gray-50 p-1.5 rounded transition-colors"
